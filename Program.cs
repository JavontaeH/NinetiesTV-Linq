...
namespace NinetiesTV
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Print("Genres of the 80s", GenresOfThe80s(shows));
            Print("List of Genres", ListOfGenres(shows));
            Print("Shows Per Year", ShowsPerYear(shows));
            Print("Total Watch Time", TotalWatchTime(shows));
            Print("Best Year Rating", BestYearRating(shows));
        }
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Average(p => p.ImdbRating);
        }
        ...
        static List<Show> OnlyInNineties(List<Show> shows)
        {
            ...
            return shows.Where(s => s.StartYear.ToString().StartsWith("199") || s.EndYear.ToString().StartsWith("199")).ToList();
        }
        ...
        {
            return shows.Where(s => s.EpisodeCount == shows.Max(s => s.EpisodeCount)).First();
        }
        ...
        {
            return shows.OrderBy(s => s.EndYear).Where(s => s.EndYear.ToString().StartsWith("200")).First();
        }
        ...
        {
            return shows.Where(s => s.Name.Length == shows.Max(s => s.Name.Length)).First();
        }
        ...
        {
            return shows.Select(s => s.Name).Aggregate((current, next) => current + ", " + next);
        }
        ...
        {
            return shows.Select(s => s.Name).Take(shows.Count - 1).Aggregate((current, next) => current + ", " + next) + " and " + shows.Select(s => s.Name).Last();
        }
        ...
        **************************************************************************************************/
        ...
        static List<String> GenresOfThe80s(List<Show> shows)
        {
            var shows80s = shows.Where(s => s.StartYear.ToString().StartsWith("198") || s.EndYear.ToString().StartsWith("198")).ToList();
            return shows80s.SelectMany(s => s.Genres).Distinct().ToList();
        }
        ...
        static List<String> ShowsPerYear(List<Show> shows)
        {
            ...
            for (double i = shows.Min(s => s.StartYear); i <= shows.Max(s => s.EndYear); i++)
            {
                string str = $"{i}: " + shows.Where(s => s.StartYear == i).Count();
                resultList.Add(str);
            }
            return resultList;
        }
        ...
        static string TotalWatchTime(List<Show> shows)
        {
            var comedyWatchTime = shows.Where(s => s.Genres.Contains("Comedy")).Sum(s => s.EpisodeCount) * 22;
            var othersWatchTime = shows.Where(s => (s.Genres.Contains("Comedy") == false)).Sum(s => s.EpisodeCount) * 42;
            return othersWatchTime + comedyWatchTime + " minutes";
        ...
        static string BestYearRating(List<Show> shows)
        {
            List<double> ratings = new List<double>();
            List<double> years = new List<double>();
            for (double i = shows.Min(s => s.StartYear); i <= shows.Max(s => s.EndYear); i++)
            {
                double rating = shows.Where(s => s.StartYear <= i && s.EndYear >= i).Average(s => s.ImdbRating);
                ratings.Add(rating);
                years.Add(i);
            }
            double maxValue = ratings.Max();
            return $"{years[ratings.IndexOf(maxValue)]}: {maxValue}";
        }
